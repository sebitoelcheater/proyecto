...
public class Controlador  //NOTA: ESCRIBIR LOS METODOS NECESARIOS PARA EL CONTROLADOR(NO OLVIDAR QUE EL CONTROLADOR ES COMO UNA CLASE BIBLIOTECA NADA MAS)
{
	...
	static public ArrayList<Modulo> obtenerLosModulosProximosFin(Context context,Calendar ahora, int minutos) //TERMINAR
	{
		ArrayList<Modulo> modulos = new ArrayList<Modulo>();
		Calendar despues = ((Calendar)ahora.clone());
		despues.add(Calendar.MINUTE,minutos);
		...
		if(despues.get(Calendar.DAY_OF_WEEK)== ahora.get(Calendar.DAY_OF_WEEK))
		{
			Cursor c = db.getModulosFinEntreHORARIOS(ahora.get(Calendar.DAY_OF_WEEK)+"", stringAhora, stringDespues);
			if (c.moveToFirst())
			{
				do {
					String idmodulo = c.getString(0);
		...
		{
			Cursor c = db.getModulosFinEntreHORARIOS(ahora.get(Calendar.DAY_OF_WEEK)+"", stringAhora, "23:59");
			if (c.moveToFirst())
			{
				do {
					String idmodulo = c.getString(0);
			...
			c = db.getModulosFinEntreHORARIOS(despues.get(Calendar.DAY_OF_WEEK)+"", "00:00", stringDespues);
			if (c.moveToFirst())
			{
				do {
					String idmodulo = c.getString(0);
