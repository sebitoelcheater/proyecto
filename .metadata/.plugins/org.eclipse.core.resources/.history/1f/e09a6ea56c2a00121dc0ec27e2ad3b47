...
public class AdapterCursos {
    ...
    private static final String TABLE_CREATE_CURSOS =
        "create table if not exists Cursos (iidC integer primary key autoincrement, idC integer, idP integer, title VARCHAR not null, comentable integer);";
        
    private static final String TABLE_CREATE_HORARIOS =
        "create table if not exists Horarios (iidH integer primary key autoincrement, idH integer, idC integer, dds integer, inicio date, fin date, ubicacion VARCHAR);";
    ...
    private static final String TABLE_CREATE_COMENTARIOS =
        "create table if not exists Comentarios (iidCom integer primary key autoincrement, idCom integer, idH integer, fecha date, comentario VARCHAR);";
    ...
    public long insertRecordCURSOS(int idC, int idP, String title, int comentable) 
    {
    ...
    {
    	Cursor mCursor =
                db.query(DATABASE_TABLE_CURSOS, new String[] {KEY_ROWIIDC, KEY_ROWIDC, KEY_ROWIDP, 
    ...
    //---updates a record---
    public boolean updateRecordCURSOS(long rowId, String idC, String idP, String title, int comentable) 
    {
        ContentValues args = new ContentValues();
        args.put(KEY_ROWIDC, idC);
        args.put(KEY_ROWIDP, idP);
        args.put(KEY_TITLE, title);
        args.put(KEY_COMENTABLE, comentable);
        return db.update(DATABASE_TABLE_CURSOS, args, KEY_ROWIIDC + "=" + rowId, null) > 0;
    }
    
    public long insertRecordHORARIOS(int idH,int idC,String diaDeLaSemana,String inicio, String fin, String ubicacion) 
    {
        ContentValues initialValues = new ContentValues();
    ...
    {
        Cursor mCursor =
                db.query(true, DATABASE_TABLE_HORARIOS, new String[] {KEY_ROWIIDH, KEY_ROWIDH, KEY_ROWIDC, KEY_DDS,
    ...
    public Cursor getRecordPorCursoHORARIOS(String idCurso)
    {
    	 Cursor mCursor =
                 db.query(true, DATABASE_TABLE_HORARIOS, new String[] {KEY_ROWIIDH, KEY_ROWIDH, KEY_ROWIDC, KEY_DDS,
                 KEY_INICIO, KEY_FIN, KEY_UBICACION}, 
                 KEY_ROWIDC + "=" + idCurso, null, null, null, null, null);
         if (mCursor != null) {
             mCursor.moveToFirst();
         }
         return mCursor;
